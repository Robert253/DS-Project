...
import org.DS.robertproject.SmartBuildingGRPC.LampServiceGrpc.LampServiceImplBase;
import org.DS.robertproject.SmartBuildingGRPC.boolRequest;
import org.DS.robertproject.SmartBuildingGRPC.boolResponse;
import org.DS.robertproject.SmartBuildingGRPC.lampResp;
...
public class LampServer extends LampServiceImplBase{
	...
	public void initialDevice(Empty request, StreamObserver<lampResp> responseObserver) {
		...
		String aname = myLight.getDeviceName();
		String aStatus = status;
		Integer aBrightness = myLight.getBrightness();
		
		//print out
		lampResp response = lampResp.newBuilder().setAname(aname).setStatus(aStatus).setBrightness(aBrightness).build();
		responseObserver.onNext(response);
		responseObserver.onCompleted();
	}

	@Override
	public void changeBrightness(valRequest request, StreamObserver<valResponse> responseObserver) {//start changeBrightness method
		// TODO Auto-generated method stub
		...
		if(newBrightness > 100 || newBrightness < 0 ) {//start if
			...
			valResponse response = valResponse.newBuilder().setLength(myLight.getBrightness()).build();
			responseObserver.onNext(response);
			responseObserver.onCompleted();
		}//end if
		else {//start else
	...
	@Override
	public void onOff(boolRequest request, StreamObserver<boolResponse> responseObserver) {//start onOff method
		// TODO Auto-generated method stub
